<?php
    ...
    if ( !class_exists( 'AvailabilityHelper' ) ) {
        class AvailabilityHelper
        {
            ...
            public function insert_calendar_bulk( $data, $posts_per_page, $total, $current_page, $all_days, $post_id )
            {
                ...
                if ( $data[ 'groupday' ] == 0 ) {
                    for ( $i = $start; $i <= $end; $i++ ) {
                        ...
                        $this->traveler_insert_availability( $data[ 'post_id' ], $data[ 'start' ], $data[ 'end' ], $data[ 'price' ], $data[ 'adult_price' ], $data[ 'children_price' ], $data[ 'infant_price' ], $data[ 'starttime' ], $data[ 'status' ], $data[ 'groupday' ] );
                    }
                } else {
                    for ( $i = $start; $i <= $end; $i++ ) {
                        $data[ 'start' ] = $all_days[ $i ][ 'min' ];
            ...
            public function traveler_insert_availability( $post_id = '', $check_in = '', $check_out = '', $price = '', $adult_price = '', $children_price = '', $infant_price = '', $starttime = '', $status = '', $group_day = '' )
            {
                global $wpdb;
					        ...
					        if ( $val->status == 'available' ) {
						        if ( intval( $val->groupday ) == 1 ) {
							        $results[] = [
								        ...
								        'start'        => date( 'Y-m-d', $val->check_in ),
								        'end'          => date( 'Y-m-d', strtotime( '+1 day', $val->check_out ) ),
								        'price'  => ( (float)$val->price < 0 ) ? 0 : (float)$val->price,
								        'status'       => 'available'
							        ];
						        } else {
							        for ( $i = $val->check_in; $i <= $val->check_out; $i = strtotime( '+1 day', $i ) ) {
                                ...
                                if ( !$in_item ) {
                                    ...
                                    if ( $i >= $today && $sub_total > 0 && $period >= $booking_period ) {
                                        $results[] = [
                                            ...
                                            'day'          => date( 'd', $i ),
                                            'date'         => date( 'Y-m-d', $i ),
                                            'base_price'  => ( $base_price ) ? TravelHelper::format_money( $base_price ) : __( 'Free', ST_TEXTDOMAIN ),
                    ...
                    if ( is_array( $data_activity ) && count( $data_activity ) ) {
                        ...
                        foreach ( $data_activity as $key => $val ) {
                            if ( $val->status == 'available' ) {
                                ...
                                if ( $max_people > 0 ) {
                                    $stotal = 1;
                                    $starttime = self::_get_activity_starttime_by_date($activity_id, $val->check_in);
                                    if($starttime != ''){
                                        $starttime = explode(', ', $starttime);
                                        $stotal = 0;
                                        if(!empty($starttime)){
                                            foreach ($starttime as $kk => $vv){
                                                $free_people_arr = ActivityHelper::_get_activity_free_peple_by_time($activity_id, $val->check_in,$val->check_out, $vv);
                                                if(!empty($free_people_arr)){
                                                    $free_people = intval( $free_people_arr[ 'free_people' ] );
                                                    $stotal   = $stotal + $free_people;
                                                }else{
                                                    $stotal = $max_people;
                                                }
                                            }
                                ...
                                if ( intval( $val->groupday ) == 1 ) {
                                    if ( $val->check_in >= $today && count( $cant_book ) <= 0 && $period >= $booking_period && $stotal > 0) {
                                        $results[] = [
                                            'start'        => date( 'Y-m-d', $val->check_in ),
                                            'day'          => date( 'd', $val->check_in ),
                                            'date'         => date( 'Y-m-d', $val->check_in ),
	        ...
	        public function _add_custom_price_rental(){
		        ...
		        for ( $i = $check_in; $i <= $check_out; $i = strtotime( '+1 day', $i ) ) {
			        $list_rental = self::_getdataRentalEachDate( $post_id, $i, $i );
			        if ( is_array( $list_rental ) && count( $list_rental ) ) {
				        foreach ( $list_rental as $key => $val ) {
					        if ( $i == $val->check_in ) {
						        if ( intval( $val->check_out ) - intval( $val->check_in ) == 0 ) {
							        self::_deleteData( $val->id );
						        } else {
            ...
            public function _add_custom_price_tour()
            {
                ...
                $check_in  = STInput::request( 'calendar_check_in', '' );
                $check_out = STInput::request( 'calendar_check_out', '' );
	            $price_type = STInput::request( 'calendar_price_type', 'person' );
                if ( empty( $check_in ) || empty( $check_out ) ) {
                    echo json_encode( [
                        'type'    => 'error',
                        'status'  => 0,
