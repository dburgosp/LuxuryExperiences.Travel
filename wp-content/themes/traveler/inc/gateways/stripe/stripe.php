...
if (!class_exists('ST_Stripe_Payment_Gateway')) {
	class ST_Stripe_Payment_Gateway extends STAbstactPaymentGateway
	{
		...
		function _load_scripts(){
            wp_enqueue_style('stripe-css', get_template_directory_uri() . '/css/stripe.css');
        }
		...
		function get_option_fields()
		{
			return array(
                ...
                array(
                    ...
                    'label'     => __('Enable Using Token', ST_TEXTDOMAIN),
                    'type'      => 'on-off',
                    'section'   => 'option_pmgateway',
                    'std'       => 'off',
                    'desc'      => __('Allow you to use token for stripe', ST_TEXTDOMAIN),
                    'condition' => 'pm_gway_st_stripe_enable:is(on)'
                ),
