...
if(!class_exists('ST_Flights_Controller')){
    class ST_Flights_Controller{
        ...
        public function traveler_get_availability( $post_id = '', $check_in = '', $check_out = '' )
        {
            ...
            $sql = "SELECT * FROM {$table} WHERE post_id = {$post_id} AND ( ( CAST( start_date AS UNSIGNED ) >= CAST( {$check_in} AS UNSIGNED) AND CAST( start_date AS UNSIGNED ) <= CAST( {$check_out} AS UNSIGNED ) ) OR ( CAST( end_date AS UNSIGNED ) >= CAST( {$check_in} AS UNSIGNED ) AND ( CAST( end_date AS UNSIGNED ) <= CAST( {$check_out} AS UNSIGNED ) ) ) )";
        ...
        public function st_filter_flights_ajax(){
            ...
            $action = STInput::get('action');
            $price_range = STInput::get('price_range');
            $stops = STInput::get('stops');
            $airline = STInput::get('airline');
            $dp_time = STInput::get('dp_time');
            ...
            //End content single item

            ob_start();
            /*echo 'Action: ' . $action . '<br />';
            echo 'Price: ' . $price_range . '<br />';
            echo 'Tax Stop: ' . $stops . '<br />';
            echo 'Tax Air: ' . $airline . '<br />';
            echo 'Tax Time: ' . $dp_time . '<br />';
